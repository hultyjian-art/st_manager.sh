...
#          即可浏览手机公共存储中的任意文件夹 —— Termux 的 targetSdk=28 + 
#          termux-setup-storage 使 Node 可以直接按路径读取 /storage/emulated/0)
#       ③ 其余逻辑 (依赖检测/端口处理/备份/多实例/开机自启) 与原版一致
# 版本: v1.3.5-luker.1
...
# 默认安装目錄與實例管理
ST_MAIN_DIR="$HOME/Luker"
ST_SECOND_DIR="$HOME/Luker-2"
ST_DIR="$ST_MAIN_DIR"
CURRENT_INSTANCE="主程序 (Luker)"

REPO_URL="https://github.com/funnycups/Luker.git"
BACKUP_DIR="$HOME/st_backups"
SCRIPT_VERSION="v1.3.5-luker.1"
SCRIPT_URL="https://raw.githubusercontent.com/hultyjian-art/st_manager.sh/main/luker_toolbox.sh"
...
# Luker 版新增: 预装图床后端插件 (本地图片展示脚本的后端)
# 提供 /roots /tree /file /thumb /diag 等端点, 让"图库"能浏览并免导入直插
# 手机存储中的任意文件夹 (Termux 授权后 Node 可直接读 /storage/emulated/0)
function install_lig_plugin() {
    local plugin_dir="$ST_DIR/plugins/lig-local-images"
    if [ -d "$plugin_dir/.git" ]; then
        print_info "图床后端插件已存在, 跳过预装。"
        return
    fi
    print_info "正在预装图床后端插件 lig-local-images..."
    if git clone --depth 1 https://github.com/hultyjian-art/lig-local-images.git "$plugin_dir"; then
        print_info "图床后端插件安装完成 (Luker 的 enableServerPlugins 默认开启, 重启酒馆即生效)。"
        print_info "在酒馆'图库'里注册手机存储中的任意文件夹, 即可浏览缩略图并免导入直插。"
        print_warn "前端界面脚本仍需手动导入酒馆助手: 包名 '酒馆助手脚本-本地图片展示-阶段4合并版.json' (见项目 README)。"
    else
        print_warn "图床后端插件预装失败 (不影响酒馆本体)。可稍后在酒馆插件管理里手动安装:"
        print_warn "  https://github.com/hultyjian-art/lig-local-images"
    fi
}
...
function install_st() {
    ...
    if npm install; then
        install_lig_plugin
        print_info "安装完成！你可以选择 '启动 Luker' 来运行。"
    else
...
function fix_host_whitelist_security() {
    ...
    print_warn "安全提醒：Luker 1.13.4 以下版本存在已知安全风险，请尽快升级到 1.13.4 及以上版本。"
...
' "$config_file" > "$tmp_file"; then
        ...
        print_warn "再次提醒：请确保 Luker 版本升级到 1.13.4 或更高版本。"
    else
        rm -f "$tmp_file"
...
function reinstall_dependencies() {
    ...
    print_warn "此操作将重新下载并安装 Luker 的运行依赖 (node_modules)。"
...
function start_st() {
    ...
    print_info "正在启动 Luker..."
    if [ ! -f "start.sh" ]; then
        print_error "未找到 start.sh，无法启动 Luker。"
        return
    fi
    bash start.sh
}
...
if [ -z "$TMUX" ] && [ -z "$ANGLER_SESSION_GUARD" ]; then
    ...
    if [ -f "$HOME/luker_toolbox.sh" ]; then
        bash "$HOME/luker_toolbox.sh"
    fi
fi
...
function disable_autostart() {
    ...
    for RC_FILE in "${CONFIG_FILES[@]}"; do
        if [ -f "$RC_FILE" ]; then
            ...
            # 查找包含 luker_toolbox.sh 的行，且不是注释行(虽然 sed 会删掉整行)
            if grep -q "luker_toolbox.sh" "$RC_FILE" 2>/dev/null; then
                # 备份文件
                cp "$RC_FILE" "${RC_FILE}.bak_$(date +%s)"
                # 删除包含脚本名的行
                sed -i '/luker_toolbox.sh/d' "$RC_FILE"
...
function do_uninstall_st() {
    if [ -d "$ST_DIR" ]; then
        print_info "正在删除 Luker 目录..."
        rm -rf "$ST_DIR"
        print_info "Luker 已卸载。"
    else
        print_error "Luker 未安装。"
    fi
}
...
function do_uninstall_script() {
    disable_autostart
    local script_path="$HOME/luker_toolbox.sh"
...
function uninstall_all() {
    if safe_verify "卸载 Luker 和 管理脚本"; then
        do_uninstall_st
        do_uninstall_script
    fi
}
...
function main_menu() {
    while true; do
        ...
        echo -e " ${GREEN}1.${NC} 启动 Luker       ${GREEN}2.${NC} 安装 Luker"
        echo -e " ${GREEN}3.${NC} 更新 Luker       ${GREEN}4.${NC} 版本回退/切换"
...
function check_first_run_autostart() {
    ...
    for f in "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.bash_profile" "$HOME/.profile" "$HOME/.zprofile" "$HOME/.bash_login"; do
        if [ -f "$f" ]; then
            count=$(grep -c "luker_toolbox.sh" "$f" 2>/dev/null)
